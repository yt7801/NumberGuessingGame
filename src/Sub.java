...
public class Sub {
	public static void main(String[] args) {
		...
		int bestScore = Integer.MAX_VALUE; //ベストスコアの初期値（大きな値）
		...
		do {
			...
			// 答えが外れていて、かつ回数が残っている間繰り返す
					...
					try {
						...
						if (guess < answer) {
							...
							if (attempts < bestScore) {
								bestScore = attempts;
								System.out.println("新記録です！ ベストスコア：" + bestScore + " 回");
							}
							break;
						}
				...
				if (guess != answer) {
					...
					System.out.println("ベストスコア:" + (bestScore == Integer.MAX_VALUE ? "記録なし" : bestScore + " 回"));
				}
